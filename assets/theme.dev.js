...
(function (AOS, FlickityFade, scrollLock, Flickity, Sqrl, MicroModal, Rellax, themeCurrency, axios, FlickitySync, themeAddresses) {
    ...
    class Media {
      ...
      doFlktyChange(index) {
        ...
        setTimeout(() => {
          if (currentThumb?.classList.contains(classes$i.hide)) {
    ...
    class Quantity {
      constructor(wrapper) {
        ...
        const qtyBreaksAttribute = this.wrapper.getAttribute('data-qty-breaks');
        this.steps = qtyBreaksAttribute ? JSON.parse(qtyBreaksAttribute) : null;
        if(this.input) {
      ...
      getNextValue(currentValue, increase) {
        if (this.steps) { 
          if (increase) {
            for (let i = 0; i < this.steps.length; i++) {
              if (currentValue < this.steps[i]) {
                return this.steps[i];
              }
            }
          ...
          return increase ? currentValue + 1 : Math.max(this.min, currentValue - 1);
        }
      }
      ...
      initButtons() {
        this.increase.addEventListener(
          ...
          function (e) {
            ...
            currentValue = isNaN(currentValue) ? this.min : currentValue;
            this.input.value = this.getNextValue(currentValue, true);
        ...
        this.decrease.addEventListener(
          ...
          function (e) {
            ...
            let currentValue = parseInt(this.input.value, 10);
            currentValue = isNaN(currentValue) ? this.min : currentValue;
            this.input.value = this.getNextValue(currentValue, false);
            this.input.dispatchEvent(new Event('change'));
          }.bind(this)
        );
      }
